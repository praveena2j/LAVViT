...
def _wav2fbank(opt, norm_mean, norm_std, filename, filename2=None, idx=None):
	# mixup
	if filename2 == None:
...
class train_loader(object):
	def __init__(self, opt, mode='train'):
		...
		if self.opt.vis_encoder_type == 'vit':
			#self.norm_mean = -6.614767551422119 #-6.2668
			#self.norm_std = 3.11063814163208 #3.2946
			self.norm_mean = -6.335798740386963
			self.norm_std = 3.234133720397949
			#mean:  -6.335798740386963 std:  3.234133720397949
		### <----
		
		elif self.opt.vis_encoder_type == 'swin':
			## ---> yb calculate: AVE dataset for 192
			self.norm_mean =  -7.174656391143799 #-7.405336856842041 #-7.020560264587402 # -4.984795570373535
			self.norm_std =  3.5024380683898926 #3.422518014907837 #3.620952606201172 #3.7079780101776123
			## <----
	...
	def load_wav(self, file_name):
		...
		for audio_sec in range(10):
			...
			fbank, mix_lambda = _wav2fbank(self.opt, self.norm_mean, self.norm_std, wave_file_path, idx=audio_sec)
	...
	def load_face(self, file):
		...
		for vis_idx in range(10):
			...
			img_path = self.opt.train_video_folder+comps1[0]+'/'+comps1[2]+'/'+comps1[3][:-4]+'/'+str("{:02d}".format(tmp_idx))+ '.jpg'
			if not os.path.isfile(img_path):
				img_path = self.opt.train_video_folder+comps1[0]+'/'+comps1[2]+'/'+comps1[3][:-4]+'/'+str("{:02d}".format(tmp_idx-1))+ '.jpg'
			if not os.path.isfile(img_path):
				img_path = self.opt.train_video_folder+comps1[0]+'/'+comps1[2]+'/'+comps1[3][:-4]+'/'+str("{:02d}".format(tmp_idx+1))+ '.jpg'
			tmp_img = torchvision.io.read_image(img_path)/255
...
class eval_loader(object):
	def __init__(self, opt, mode):  
		...
		### <----
			...
			## ---> yb calculate: AVE dataset for 192
			## ---> yb calculate: AVE dataset for 192
			self.norm_mean =  -7.405336856842041 #-7.020560264587402 # -4.984795570373535
			self.norm_std =  3.422518014907837 #3.620952606201172 #3.7079780101776123
	...
	def __getitem__(self, index):
		...
		for num in range(len(data_lists)):
			...
			for audio_sec in range(10):
				...
				fbank, mix_lambda = _wav2fbank(self.opt, self.norm_mean, self.norm_std, os.path.join(self.opt.train_audio_folder,self.ids_to_names[comps1[0]],comps1[2],comps1[3].zfill(9)), idx=audio_sec)
				total_audio.append(fbank)
			total_audio = torch.stack(total_audio)
			#return total_audio
			...
			for vis_idx in range(10):
				...
				tmp_img = torchvision.io.read_image(self.eval_path+comps2[0]+'/'+comps2[2]+'/'+comps2[3][:-4]+'/'+str("{:02d}".format(tmp_idx))+ '.jpg')/255
				tmp_img = self.my_normalize(tmp_img)
				total_img.append(tmp_img)
			total_img = torch.stack(total_img)
